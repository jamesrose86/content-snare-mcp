...
function getClientCredentials() {
    const clientId = process.env.CONTENTSNARE_CLIENT_ID;
    const clientSecret = process.env.CONTENTSNARE_CLIENT_SECRET;
    if (!clientId || !clientSecret) {
        throw new Error("CONTENTSNARE_CLIENT_ID and CONTENTSNARE_CLIENT_SECRET environment variables are required. " +
            "Create an API application in Content Snare under Settings > API to obtain these.");
    }
...
async function refreshAccessToken(refreshToken, clientId, clientSecret) {
    ...
    if (!res.ok) {
        ...
        throw new Error(`Token refresh failed (${res.status}): ${text}. ` +
            "Run 'contentsnare-mcp auth' to re-authorize.");
    }
    const data = (await res.json());
    return {
        access_token: data.access_token,
...
// ---------------------------------------------------------------------------
// Token resolution: env var OR stored tokens with auto-refresh
// ---------------------------------------------------------------------------
async function getToken() {
    // Direct token takes precedence (for users who manage tokens themselves)
    const envToken = process.env.CONTENTSNARE_ACCESS_TOKEN;
    if (envToken)
        return envToken;
    // Otherwise use stored OAuth tokens
    let tokens = loadTokens();
    if (!tokens) {
        throw new Error("Not authenticated. Either:\n" +
            "  1. Run 'npx contentsnare-mcp auth' to authorize via browser, or\n" +
            "  2. Set CONTENTSNARE_ACCESS_TOKEN environment variable.");
    }
...
async function runAuthFlow() {
    ...
    if (!redirectUri) {
        throw new Error("CONTENTSNARE_REDIRECT_URI environment variable is required for authorization.\n" +
    ...
    const authUrl = `${OAUTH_BASE}/oauth/authorization?` +
...
async function api(token, method, path, body, query) {
    ...
    if (!res.ok) {
        const errMsg = typeof data === "object" && data && "errors" in data
            ? data.errors.join("; ")
            : typeof data === "string"
                ? data
                : JSON.stringify(data);
...
const Question = z.object({
    ...
    options: z
        ...
        .nullable()
        .optional()
        .describe("Array of option strings. Required for: single select option, multi select options, task list, dropdown"),
    columns: z
        .array(z.string())
        .nullable()
        .optional()
...
// ---------------------------------------------------------------------------
// CLI entry point: handle "auth" subcommand or start MCP server
// ---------------------------------------------------------------------------
...
function startServer() {
    const server = new McpServer({
        name: "contentsnare",
        version: "1.0.0",
        description: "MCP server for the Content Snare API. Manage clients, requests, templates, team members, webhooks, and more.",
    });
    // ---------------------------------------------------------------------------
    // Tool: me
    // ---------------------------------------------------------------------------
    ...
    // Tools: Clients
    // ---------------------------------------------------------------------------
    server.tool("list_clients", "List active clients. Scopes: read_clients", {
        ...PaginationParams,
        ...SortParams,
        ...SearchParams,
        include_external: z
        ...
        if (params.q)
            query.q = params.q;
        if (params.q_by)
            query["q_by[]"] = params.q_by;
        if (params.include_external !== undefined)
            query.include_external = String(params.include_external);
        const token = await getToken();
        const { data } = await api(token, "GET", "/clients", undefined, query);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("get_client", "Get a client by ID. Scopes: read_clients", { id: z.string().describe("Client ID") }, async ({ id }) => {
        const token = await getToken();
        const { data } = await api(token, "GET", `/clients/${id}`);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("create_client", "Create a new client. Scopes: write_clients", {
        ...
        const token = await getToken();
        const { data } = await api(token, "POST", "/clients", params);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("update_client", "Update an existing client. Scopes: write_clients", {
        ...
        const token = await getToken();
        const { data } = await api(token, "PUT", `/clients/${id}`, body);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("delete_client", "Delete a client. Scopes: write_clients", { id: z.string().describe("Client ID") }, async ({ id }) => {
        const token = await getToken();
        await api(token, "DELETE", `/clients/${id}`);
        return { content: [{ type: "text", text: "Client deleted successfully." }] };
    });
    // ---------------------------------------------------------------------------
    // Tools: Client Companies
    // ---------------------------------------------------------------------------
    server.tool("list_client_companies", "List client companies. Scopes: read_clients", {
        ...PaginationParams,
        ...SortParams,
        ...SearchParams,
    }, async (params) => {
        ...
        if (params.q_by)
            query["q_by[]"] = params.q_by;
        const token = await getToken();
        const { data } = await api(token, "GET", "/client_companies", undefined, query);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    // ---------------------------------------------------------------------------
    // Tools: Requests
    // ---------------------------------------------------------------------------
    server.tool("list_requests", "List requests. Scopes: read_requests", {
        ...PaginationParams,
        ...SortParams,
        ...SearchParams,
        filter_statuses: z
        ...
        if (params.filter_statuses)
            query["filter_by[statuses][]"] = params.filter_statuses;
        if (params.expand)
            query["expand[]"] = params.expand;
        const token = await getToken();
        const { data } = await api(token, "GET", "/requests", undefined, query);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("get_request", "Get a request by ID, including its pages. Scopes: read_requests", { id: z.string().describe("Request ID") }, async ({ id }) => {
        const token = await getToken();
        const { data } = await api(token, "GET", `/requests/${id}`);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("create_request", "Create a new request from a template. Scopes: write_requests, write_clients (if client is new)", {
        ...
        const token = await getToken();
        const { data } = await api(token, "POST", "/requests", params);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("create_request_with_fields", "Create a request with inline pages, sections, and fields (no template needed). " +
        ...
        pages_data: z
            .array(PageData)
            .describe("Array of pages, each containing sections with questions. " +
    ...
    server.tool("update_request", "Update an existing request. Scopes: write_requests", {
        ...
        const token = await getToken();
        const { data } = await api(token, "PUT", `/requests/${id}`, body);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("delete_request", "Delete a request. Scopes: write_requests", { id: z.string().describe("Request ID") }, async ({ id }) => {
        const token = await getToken();
        await api(token, "DELETE", `/requests/${id}`);
        return { content: [{ type: "text", text: "Request deleted successfully." }] };
    });
    // ---------------------------------------------------------------------------
    // Tools: Request bulk actions
    // ---------------------------------------------------------------------------
    server.tool("approve_all_submitted_fields", "Approve all submitted fields in a request, page, or section. Scopes: review_requests", {
        scope: z
        ...
        const token = await getToken();
        await api(token, "PUT", `/${pathMap[scope]}/${id}/approve_all_submitted_fields`);
        return {
            content: [
                {
                    type: "text",
    ...
    server.tool("submit_all_fields", "Submit all fields for review in a request, page, or section. Scopes: review_requests", {
        scope: z
            .enum(["request", "page", "section"])
        ...
        const pathMap = { request: "requests", page: "pages", section: "sections" };
        const token = await getToken();
        await api(token, "PUT", `/${pathMap[scope]}/${id}/submit_all_fields`);
        return {
            content: [
                {
                    type: "text",
    ...
    server.tool("run_integration_actions", "Run integration actions inside a request. Scopes: review_requests", { id: z.string().describe("Request ID") }, async ({ id }) => {
        const token = await getToken();
        await api(token, "PUT", `/requests/${id}/run_integration_actions`);
        return {
    ...
    // Tools: Pages
    // ---------------------------------------------------------------------------
    server.tool("get_page", "Get a page by ID, including its sections and fields. Scopes: read_requests", {
        ...
        const token = await getToken();
        const { data } = await api(token, "GET", `/pages/${id}`, undefined, query);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("create_page", "Create a new page in a request from a template. Scopes: write_requests", {
        ...
        const token = await getToken();
        const { data } = await api(token, "POST", "/pages", params);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    // ---------------------------------------------------------------------------
    // Tools: Sections
    // ---------------------------------------------------------------------------
    server.tool("create_section", "Create a new section in a page from a template. Scopes: write_requests", {
        ...
        const token = await getToken();
        const { data } = await api(token, "POST", "/sections", params);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    // ---------------------------------------------------------------------------
    // Tools: Fields
    // ---------------------------------------------------------------------------
    server.tool("review_field", "Review a field - approve, reject, submit for review, or revise. Scopes: review_requests", {
        id: z.string().describe("Field ID"),
        status: z
            .enum(["approved", "done", "redo", "todo"])
            .describe("Field status: 'approved' to approve, 'done' to submit for review, " +
        ...
        const token = await getToken();
        const { data } = await api(token, "PUT", `/fields/${id}/review`, body);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    // ---------------------------------------------------------------------------
    // Tools: Templates
    // ---------------------------------------------------------------------------
    server.tool("list_request_templates", "List request templates. Scopes: read_templates", PaginationParams, async (params) => {
        const query = {};
        if (params.limit !== undefined)
            query.limit = String(params.limit);
        if (params.offset !== undefined)
            query.offset = String(params.offset);
        const token = await getToken();
        const { data } = await api(token, "GET", "/request_templates", undefined, query);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("list_page_templates", "List page templates. Scopes: read_templates", PaginationParams, async (params) => {
        const query = {};
        if (params.limit !== undefined)
            query.limit = String(params.limit);
        if (params.offset !== undefined)
            query.offset = String(params.offset);
        const token = await getToken();
        const { data } = await api(token, "GET", "/page_templates", undefined, query);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("list_section_templates", "List section templates. Scopes: read_templates", PaginationParams, async (params) => {
        const query = {};
        if (params.limit !== undefined)
            query.limit = String(params.limit);
        if (params.offset !== undefined)
            query.offset = String(params.offset);
        const token = await getToken();
        const { data } = await api(token, "GET", "/section_templates", undefined, query);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    // ---------------------------------------------------------------------------
    // Tools: Communication Templates
    // ---------------------------------------------------------------------------
    server.tool("list_communication_templates", "List communications schedules. Scopes: read_requests", PaginationParams, async (params) => {
        const query = {};
        if (params.limit !== undefined)
            query.limit = String(params.limit);
        if (params.offset !== undefined)
            query.offset = String(params.offset);
        const token = await getToken();
        const { data } = await api(token, "GET", "/communication_templates", undefined, query);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    // ---------------------------------------------------------------------------
    // Tools: Folders
    // ---------------------------------------------------------------------------
    server.tool("list_folders", "List folders. Scopes: read_requests", PaginationParams, async (params) => {
        const query = {};
        if (params.limit !== undefined)
            query.limit = String(params.limit);
        if (params.offset !== undefined)
            query.offset = String(params.offset);
        const token = await getToken();
        const { data } = await api(token, "GET", "/folders", undefined, query);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    // ---------------------------------------------------------------------------
    // Tools: Team Members
    // ---------------------------------------------------------------------------
    server.tool("list_team_members", "List team members. Scopes: read_team_members", {
        ...PaginationParams,
        ...SortParams,
        ...SearchParams,
    }, async (params) => {
        ...
        if (params.q_by)
            query["q_by[]"] = params.q_by;
        const token = await getToken();
        const { data } = await api(token, "GET", "/team_members", undefined, query);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("get_team_member", "Get a team member by ID. Scopes: read_team_members", { id: z.string().describe("Team member ID") }, async ({ id }) => {
        const token = await getToken();
        const { data } = await api(token, "GET", `/team_members/${id}`);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("create_team_member", "Create a new team member. Scopes: write_team_members", {
        ...
        const token = await getToken();
        const { data } = await api(token, "POST", "/team_members", params);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("update_team_member", "Update a team member. Scopes: write_team_members", {
        ...
        const token = await getToken();
        const { data } = await api(token, "PUT", `/team_members/${id}`, body);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("delete_team_member", "Delete a team member. Scopes: write_team_members", { id: z.string().describe("Team member ID") }, async ({ id }) => {
        const token = await getToken();
        await api(token, "DELETE", `/team_members/${id}`);
    ...
    server.tool("list_webhooks", "List webhooks. Scopes: administration", PaginationParams, async (params) => {
        const query = {};
        if (params.limit !== undefined)
            query.limit = String(params.limit);
        if (params.offset !== undefined)
            query.offset = String(params.offset);
        const token = await getToken();
        const { data } = await api(token, "GET", "/webhooks", undefined, query);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("get_webhook", "Get a webhook by ID. Scopes: administration", { id: z.string().describe("Webhook ID") }, async ({ id }) => {
        const token = await getToken();
        const { data } = await api(token, "GET", `/webhooks/${id}`);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("create_webhook", "Create a new webhook. Scopes: administration", {
        ...
        const token = await getToken();
        const { data } = await api(token, "POST", "/webhooks", params);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("update_webhook", "Update a webhook. Scopes: administration", {
        ...
        const token = await getToken();
        const { data } = await api(token, "PUT", `/webhooks/${id}`, body);
        return { content: [{ type: "text", text: formatResult(data) }] };
    });
    server.tool("delete_webhook", "Delete a webhook. Scopes: administration", { id: z.string().describe("Webhook ID") }, async ({ id }) => {
        const token = await getToken();
        await api(token, "DELETE", `/webhooks/${id}`);
